...
async def rnd(ctx):
    if randint(0, 1):
        ...
        await ctx.author.add_roles(
            discord.utils.get(ctx.guild.roles, name="Vencio a la muerte"), atomic=True
        )
...
async def move_to_cinema(ctx):
    ...
    voice_channel = discord.utils.get(ctx.guild.voice_channels, id=CINEMA_CHANNEL_ID)
...
async def roles(ctx):
    ...
    embed = discord.Embed(
        ...
        description="\n".join(
            f"{role.mention} - `{counted_roles[i]} 👤`" for i, role in enumerate(roles)
        ),
    )
...
async def vote_custom(
    ctx, type_time: str, timeout: int, propuesta: str, emoji_1: str, emoji_2: str
):
    if type_time.lower() not in [type_t.lower() for type_t in TIMES]:
        await ctx.respond(
            f"Tipo de duración no reconocido `{type_time}`", ephemeral=True
    ...
    async for message in channel.history(limit=None):
        ...
        for reaction in message.reactions:
            if reaction.emoji == one_stars:
                ...
                players[members.index(message.author)].two_stars += reaction.count - 1
            elif reaction.emoji == three_stars:
                players[members.index(message.author)].three_stars += reaction.count - 1
...
async def food_ratings_custom(ctx, top: int, month: str):
    ...
    if n_month > datetime.today().month:
        await ctx.respond("No se ha inventado el viaje temporal...😅", ephemeral=True)
        return
...
async def food_statistics_of(ctx, member: discord.Member):
    ...
    async with ctx.channel.typing():
        ...
        embed.add_field(name="Media:", value=f"{player.get_mean_points()}", inline=True)
        ...
        embed.add_field(name=f"{three_stars}", value=player.three_stars, inline=True)
        embed.set_footer(
...
async def delete_ratings(ctx, id_message: str = None):
    ...
    if id_message in ("", " ", None):
        ...
        if partial_message is None:
            await ctx.respond(
                f"No se ha encontrado el mensaje con ID `{id_message}`", ephemeral=True
            )
            print_debug(
    ...
    if not (
        (ctx.author.guild_permissions.administrator) or (message.author == ctx.author)
    ):
        await ctx.respond(
...
async def russian_roulette(ctx, mode: str):
    ...
    if ctx.author.voice is None:
        ...
        await ctx.respond(
            "Debes estar en un canal de voz para jugar a la ruleta rusa", ephemeral=True
        )
        return
    ...
    if len(potential_players) < 2:
        await ctx.respond(
            "Debe de haber por lo menos dos jugadores para poder jugar", ephemeral=True
        )
        print_debug(
...
async def check_ref_queue():
    ...
    for prisoner in reformatory_cells:
        ...
        ref_channel = discord.utils.get(bot.get_all_channels(), name="⛓ Reformatorio ⛓")

        if prisoner[0].voice.channel != ref_channel:

            await prisoner[0].move_to(
                discord.utils.get(bot.get_all_channels(), name="⛓ Reformatorio ⛓")
            )
            jail_time = randrange(MIN_TIME_REFORMATORY, MAX_TIME_REFORMATORY)
